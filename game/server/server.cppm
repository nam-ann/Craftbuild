...
export namespace craftbuild {
    ...
    public:
        none _ready() override {
            ...
            log<LogType::INFO>("UDP Server listening on port 8888...");
        }
        ...
        none _process(double delta) override {
...

            {
                ...
                if (client_socket != INVALID_SOCKET) clients.push_back(client_socket);
            }

			std::vector<SOCKET> disconnected_clients;
            for (auto& client : clients) {
                uint64 buffer_size = sizeof(buffer) - 1;
                if (not receive(client, buffer, buffer_size)) {
                ...
                try {
                    if (message.content == "Connect") {
                        server.connect(message.arguments[0]);
                        send_queue.store({ "Connected" });
                    }
                    else if (message.content == "Chat") {
                        Str response = server.chat(message.arguments[0]);
                        send_queue.store({ "Chat response", { response.std_str() }});
                    }
                        ...
                        send_queue.store({ "Chunk data", { world_data } });
                    }
                        ...
                        send_queue.store({ "Updated" });
                    }
                }
                catch (const std::exception& e) {
                    log<LogType::ERROR>(format{} << "Error processing message: " << e.what());
                    send_queue.store({ "Error" });
                }

                send_queue.send(client);
            }
